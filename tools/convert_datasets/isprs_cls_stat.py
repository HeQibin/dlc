...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Stat ISPRS Dataset Label Ids')
    dataset_path = '/map-hl/qibin.he/data/rs/potsdam/irrg_512_s256'
    parser.add_argument(
        '--isprs_path', 
        default=dataset_path,
        help='isprs data path')
    parser.add_argument(
        '--gt-dir', 
...
def main():
    args = parse_args()
    isprs_path = args.isprs_path
    out_dir = args.out_dir if args.out_dir else isprs_path
    mmcv.mkdir_or_exist(out_dir)

    gt_dir = osp.join(isprs_path, args.gt_dir)
