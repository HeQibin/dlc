...
class HRDAHeadWithNTMCorrect(BaseDecodeHead):
    ...
    def __init__(self,
        ...
        super(HRDAHeadWithNTMCorrect, self).__init__(**kwargs)
        del self.conv_seg
        del self.dropout
    ...
    def forward_train(self,
        ...
        losses = self.losses(seg_logits, gt_semantic_seg, img_metas, 
                             seg_weight, target_domain=target_domain)
        if return_logits:
    ...
    def ntm_correct(self, seg_logit, ntm):
        b, c, h, w = seg_logit.shape
        seg_logit = torch.softmax(seg_logit, dim=1).permute(0, 2, 3, 1).contiguous().view(-1, c)
        seg_logit = torch.mm(seg_logit, ntm).view(b, h, w, c).permute(0, 3, 1, 2)
        return seg_logit

    def losses(self, seg_logit, seg_label, img_metas, 
               seg_weight=None, target_domain=None):
        """Compute losses."""
        fused_seg, lr_seg, hr_seg = seg_logit

        if target_domain:
            ntm = target_domain["ntm"]
            fused_seg = self.ntm_correct(fused_seg, ntm)
            lr_seg = self.ntm_correct(lr_seg, ntm)
            hr_seg = self.ntm_correct(hr_seg, ntm)

        loss = self.single_losses(fused_seg, seg_label, seg_weight, 
                                  target_domain=target_domain)

        # if target_domain:
        #     loss.update({"ntm.loss": target_domain["ntm_loss"]})
        ...
        if self.lr_loss_weight > 0:
            loss.update(
                add_prefix(
                    self.single_losses(lr_seg, seg_label,
                                                 seg_weight, target_domain=target_domain), 'lr'))
        if self.hr_loss_weight > 0 and self.enable_hr_crop:
            ...
            loss.update(
                add_prefix(
                    self.single_losses(hr_seg, cropped_seg_label,
                                                 cropped_seg_weight, target_domain=target_domain), 'hr'))
        elif self.hr_loss_weight > 0:
            loss.update(
                add_prefix(
                    self.single_losses(hr_seg, seg_label,
                                                 seg_weight, target_domain=target_domain), 'hr'))
    ...
    def single_losses(self, seg_logit, seg_label, seg_weight=None, target_domain=None):
