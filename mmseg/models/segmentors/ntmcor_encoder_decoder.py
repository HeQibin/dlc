...
class NTMCorrectEncoderDecoder(EncoderDecoder):
    last_train_crop_box = {}
    ...
    def __init__(self,
        ...
        super(NTMCorrectEncoderDecoder, self).__init__(
            backbone=backbone,
        ...
        # self.ntm = num_classes * torch.eye(num_classes) - torch.ones(num_classes)
        # self.posterior = torch.ones((num_classes, 1)) / num_classes \
        #     + torch.rand((num_classes,1))*0.1 
        # self.posterior = self.posterior.to("cuda").requires_grad_(True)
        # self.ntm = self.ntm.to("cuda").requires_grad_(True)

    # def ntm_and_posterior(self):
    #     return self.ntm, self.posterior
        ...
        if self.feature_scale in self.feature_scale_all_strs:
            ...
            for i, s in enumerate(self.scales):
                ...
                if self.crop_size is not None and i >= 1:
                    scaled_img = crop(
                        scaled_img, NTMCorrectEncoderDecoder.last_train_crop_box[i])
    ...
    def _forward_train_features(self, img):
        ...
        for i, s in enumerate(self.scales):
            ...
            if self.crop_size is not None and i >= 1:
                ...
                if self.feature_scale in self.feature_scale_all_strs:
                    NTMCorrectEncoderDecoder.last_train_crop_box[i] = crop_box
