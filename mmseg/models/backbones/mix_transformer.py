        ...
        if self.pretrained is None:
            ...
            if self.frozen_stages > 0:
                for i in range(1, self.frozen_stages + 1):
                    logger.info(f"Freeze MixTransformer stage{i}.")
                    # patch_embed_layer = getattr(self, f'patch_embed{i}')
                    # norm_layer = getattr(self, f'norm{i}')
                    # block_module = getattr(self, f'block{i}')
                    for layer_or_module in [f'patch_embed{i}', f'norm{i}', f'block{i}']:
                        lm = getattr(self, layer_or_module)
                        lm.eval()
